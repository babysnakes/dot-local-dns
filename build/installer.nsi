; NSIS template from source code (version: 0.11.4) - slightly modified to delete startup registry entries.
; Set the compression agorithm.
!if "{{compression}}" == ""
...
; Custom: my AutoLaunch registry keys
!define STARTUPRUNKEY "Software\Microsoft\Windows\CurrentVersion\Run"
!define STARTUPENABLEKEY "Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run"
...
Function PageLeaveReinstall
  ...
  reinst_uninstall:
    HideWindow
    ClearErrors
    ; Indicate to the uninstaller that this is an upgrade - do not remove startup entries.
    WriteRegStr SHCTX "${UNINSTKEY}" "RemoveStartupEntries" "0"
...
Section Install
  ...
  WriteRegStr SHCTX "${UNINSTKEY}" "RemoveStartupEntries" "1"
...
Section Uninstall
  ...
  ; Check registry key whether or not to delete startup run and act accordingly
  ReadRegStr $R8 SHCTX "${UNINSTKEY}" "RemoveStartupEntries"
  ${If} $R8 == "1"
    DeleteRegValue HKCU ${STARTUPRUNKEY} "${PRODUCTNAME}"
    DeleteRegValue HKCU ${STARTUPENABLEKEY} "${PRODUCTNAME}"
  ${Else}
    ; upgrade, nothing to do but reset the value for next run
    WriteRegStr SHCTX "${UNINSTKEY}" "RemoveStartupEntries" "1" ; reset to default
  ${EndIf}
